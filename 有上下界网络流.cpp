...
void link1(int u,int v,int c){edge[eid].v = v,edge[eid].c = c,edge[eid].next = p[u],p[u] = eid++;}
void link2(int u,int v,int c){link1(u,v,c),link1(v,u,0);}
...
bool makelevel(int s,int t)//广搜标号
{
    ...
    while(l < r){
        int u = q[l++];
        if(u==t) return true;//可达汇点
        UREP(i,u){
            int v = edge[i].v;
            if(!d[v]&&edge[i].c) q[r++] = v,d[v] = d[u] + 1;
        }
    }
    return false;
}
...
int dfs(int u,int flow,int t)
{
    ...
    if(sum==0) d[u] = -1;//表示这个点无法放流量了就在此回合屏蔽掉
    return sum;
}
...
先跑SS,TT。连<T,S,inf>，再跑SS,TT，此时T->S反向边就是最小流
去除T->S的边。跑S,T，加上这部分流量就是最大流了。
*/
