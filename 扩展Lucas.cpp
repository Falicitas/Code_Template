...
namespace Ex_LC
{
    struct node{int base,k;node(int a=0,int b=0):base(a),k(b){}};
    vector<node> fac;
    ...
    void init(int P)
    {
        ...
        if(P>1) fac.push_back(node(P,1));
        for(int i=0;i<fac.size();i++){
            bit[i][0] = 1;
            for(int j=1;j<=fac[i].k;j++) bit[i][j] = bit[i][j-1] * fac[i].base;//1e6
        }
        for(int i=0;i<fac.size();i++){
            ...
            for(int j=1;j<p_k;j++){
                tnt[i][j] = tnt[i][j-1];
                if(j % fac[i].base != 0) tnt[i][j] = 1LL * tnt[i][j] * j % p_k;
            }
        }
        _CRT::n = fac.size();
        REP(i,1,_CRT::n) _CRT::m[i] = bit[i-1][fac[i-1].k];
    }
    ...
    ll Ex_lucas(ll m,ll n,int P)//使用前记得初始化、、
    {
        for(int i=0;i<fac.size();i++){
            ...
            int p_k = bit[i][fac[i].k];
            if(del>=fac[i].k) _CRT::a[i+1] = 0;
            else _CRT::a[i+1] = 1LL * bit[i][del] * x(q_n) % p_k * inv(x(q_m),p_k) % p_k * inv(x(q_nm),p_k) % p_k;
        }
        return _CRT::CRT();
    }
}
