...
struct SegmentTree {
    ...
    SegmentTree(std::vector<Info> init)
        : SegmentTree(init.size()) {  //原来构造函数能调用构造函数。O(n)构造线段树
...
struct Info {
    // int l;//示例：这里Info维护区间中连续子段的个数，s维护该段连续的子段的个数。x=(i不是末尾 && a[i]<=a[i+1])
    // int r;
    // int len;
    // i64 s;
    // Info()
    ...
    //     : l(x), r(x), len(1), s(x + 1) {}
    /*
    这里说下 s(x+1) 的问题。由于最长连续段 [x,y) 的尾的 x=0 ，故初始化先统计 [x,y),[x+1,y),[x+2,y) 的子段
    （包括 [x,x+1),[x+1,x+2),... ，故 s = x+1）
    对于查询 [l,r] 连续子段的个数，仅查 [l,r) 即可，最后少了个 [r,r+1) ，所以结果为 rangeQuery(l,r) + 1。
    还有注意下标从 0 开始。
    */
};
